...
--        has seen: plc_srv_cd, plc_srv_ctg_cd, med_cost_ctg_cd, poa_cd,
--        mbr_state, specialty_ctg_cd.
-- WHY    Setting logic cannot be written until these come back. Raw values
--        only; nothing here maps or interprets a value - that is a numbered
--        decision in 00_docs/data_decisions.md.
...
--        here first.
--        Six queries, one per (table, column). Run them one at a time.
--        Queries 1-3 scan single columns of EMIS_CLAIM_LINE - large.
--        Query 4 scans CLM_LN_X_ICD9_DX - large. The rest are small tables.
--        No business_ln_cd or medical_ind profiles anywhere: business line
--        is out of Gate 1 entirely (DD-06), and neither is a column on the
--        curated membership - both are build filters there (DD-05).
-- ===========================================================================
...
-- Query 3: EMIS_CLAIM_LINE.med_cost_ctg_cd (dental exclusion lives here)
SELECT
...
-- Query 4: CLM_LN_X_ICD9_DX.poa_cd
SELECT
...
-- Query 5: membership mbr_state ('FL' must appear, or the Florida filter
-- in 09 finds nothing)
SELECT
...
-- Query 6: PROVIDER_DM.specialty_ctg_cd (peer grouping for steps 13-14)
SELECT
